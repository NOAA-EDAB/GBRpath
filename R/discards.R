...
#Filter observer data for GB only
#remove EPU column
...
low_var<-low_var |>
  dplyr::select(RPATH,FLEET,five_means)
names(low_var)[3]<-'DK'
...
high_var<-high_var |>
  dplyr::select(RPATH,FLEET,ten_mean)
names(high_var)[3]<-'DK'

#Put it all together for model discard values
discards<-firsts |>
  dplyr::select(RPATH,FLEET,Landings)
